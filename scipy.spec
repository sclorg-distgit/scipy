...
Patch4:  fix-arpack-tests-pr2684.patch

BuildRequires: %{?scl_prefix}numpy, %{?scl_prefix}python2-devel
...
cd test
%{?scl:scl enable %{scl} - << \EOF}
PYTHONPATH="%{buildroot}%{python_sitearch}" %{__python} -c "import scipy; scipy.test('full', verbose=3)"
%{?scl:EOF}
